...
def customerId_generator(instance,sender,*args, **kwargs):
    if not instance.customer_id:
        word="CUSTOMER"
        instance.customer_id= word+"_"+str(instance.company_name.company_Name)+str(random_string_generator())

pre_save.connect(customerId_generator,sender=CustomerNew)


class VendorNew(models.Model):
    vendor_id       = models.CharField(max_length=150, blank=True, null=True) 
    company_name           =models.ForeignKey(Company,on_delete=models.CASCADE,blank=True,null=True)
    
    def __str__(self):
        return self.company_name.company_Name

def vendorId_generator(instance,sender,*args, **kwargs):
    if not instance.vendor_id:
        word="VENDOR"
        instance.vendor_id= word+"_"+str(instance.company_name.company_Name)+"_"+str(random_string_generator())

pre_save.connect(vendorId_generator,sender=VendorNew)
